    ...
    const fetchData = async () => {
      try {
        ...
        if (mlModelResponse && mlModelResponse.status === 200) {
          ...
          // fraud Account Database Integration

          // if (currData.FraudFlag === 1) {
          //   try {
          //     // Send flagged transaction to the server
          //     await axios.post("http://localhost:5000/post/fraudTransactions", {
          //       transactionData: currData,
          //     });

          //     console.log("Flagged transaction sent to the server");
          //   } catch (error) {
          //     console.error("Error sending flagged transaction to the server:", error);
          //   }
          // }
